...
**Status:** Phases 0–6 COMPLETE ✅ — feature validated end-to-end against real cloud Supabase + real Printify (Etsy mocked). Only the **manual dashboard UI smoke** remains (needs a human at the browser; see below).
**Date:** 2026-05-30 (Phases 1–5 shipped 2026-05-31; Phase 6 integration smoke 2026-06-02)
...
> - **Phases 4 & 5 done (2026-05-31, commit `c679097`):** Phase 4 — `components/variants/VariantPicker.tsx` (controlled color+size chip multiselect, `aria-pressed`) mounted in `DesignReviewCard` after BgRemovalPicker; selection serialized as hidden JSON inputs on the Regen form; `regenerateDesign` writes `shirt_colors`/`shirt_sizes` to `trend_briefs` (empty selection SKIPs, never NULL into the NOT NULL columns); `lib/queries/variants.ts getVariantOptions(145,39)` fetched server-side. Phase 5 — `components/listings/VariantOverrideEditor.tsx` (per-variant checkboxes, select-all/none, inherit when all-checked) on the listing detail page for `needs_review`|`pending_publish`; `updateListingVariants` action (validateVariantIds subset guard, writes `selected_variant_ids` or NULL); `getEtsyPayloadPreview` filters `printify_variants` to the override so the preview reflects the narrowed set. Built via lean workflow (3 Sonnet build + Sonnet verify + 1 Opus UI/a11y review — the Opus pass caught a NULL-into-NOT-NULL Regen bug, fixed). eslint clean, tsc clean, dashboard 160 + listing 146 + shared 83 tests pass; CI green.
> - **Phase 6 integration smoke done (2026-06-02):** Ran the real forward path against cloud Supabase + real Printify with `ETSY_MOCK_MODE=true`. Seeded/approved a brief with `shirt_colors=['White','Black']`, `shirt_sizes=['S','M','L','XL','2XL']` → Design (`python -m packages.design.main`) resolved **exactly the 10 correct variant IDs** `[38163,38164,38177,38178,38191,38192,38205,38206,38219,38220]` (White+Black × 5 sizes) and wrote them to `design_packages.printify_variant_ids`. → Listing (`node --env-file=.env --import tsx/esm packages/listing/src/index.ts`) created a real hidden Printify product carrying all 10 variants, populated `printify_variants` (10 rows) + `mockups_from_actual_design=true`, paused at `needs_review`. **Override narrowing verified** by setting `listings.selected_variant_ids=[5 White IDs]` and replaying the exact `getEtsyPayloadPreview` filter + shared `buildInventoryFromDesign`: inherit → 10 inventory products (black+white); override → 5 products (white only); empty/NULL → inherit. All smoke artifacts + 3 Printify products deleted afterward; cloud restored to pre-smoke state. **⚠️ Operational lesson (acted on): the Listing agent drains ALL claimable work — a pre-run snapshot showed 0 pending listings but the run still swept up 2 unrelated half-built `pending` listings and created Printify products for them. Before running a draining agent against the live shop, explicitly park/confirm the FULL claimable queue (pending + pending_publish listings AND approved designs without listings), not just a status-count snapshot.**
> - **Phases 2 & 3 done (2026-05-31, commit `15d5ee3`):** Phase 2 — `packages/design/variant_catalog.py` `resolve_variant_ids()` queries the catalog (blueprint/provider/color IN/size IN/is_available) → sorted distinct ids; falls back to `GILDAN_64000_VARIANT_IDS` (a copy) only on empty colors/sizes or zero rows. `main.py` resolves once from `brief.shirt_colors`/`shirt_sizes` and writes at both variant-id sites. Phase 3 — `packages/shared/src/variant-selection.ts` `validateVariantIds()` (subset guard) + `VariantSelectionError`; `publisher.ts publishOne` loads `selected_variant_ids`, validates subset, passes `selected ?? design set` to `createHiddenProduct` (this is what makes the dashboard's `recreatePrintifyProduct` — which only re-queues to `pending` — apply the override); `resumePublish` filters `printify_variants` to the subset before building Etsy inventory. Empty `[]` override = inherit (never zeroes the product). Built via lean workflow (3 Sonnet build + Sonnet verify + 1 Opus logic review); tsc clean, 405 TS tests + 173 py tests pass; CI green. See [[variants-phase3-override-flow]].
...
- **Phase 1** (commit `bb78b1b`) — provider `3→39` + cost/price re-baseline ($10.09/$4.29, retail $26.99, range $25.99–$34.99) across design/listing/ledger/dashboard; migrations `054`/`055` applied to cloud; `shared/types.ts` + `shared_py/models.py` carry the new fields. Floor now $25.23.
- **Phases 2 & 3** (commit `15d5ee3`) — Design resolves `brief.shirt_colors`/`shirt_sizes` → variant IDs via `variant_catalog.py`; Listing honors the per-listing `selected_variant_ids` override (`publisher.ts` create + `resumePublish` inventory filter, guarded by shared `validateVariantIds`). All tests green; CI green.
- **Phases 4 & 5** (commit `c679097`) — dashboard `VariantPicker` on the Design review card (writes `shirt_colors`/`shirt_sizes` to the brief, survives Regen) + per-listing `VariantOverrideEditor` on the listing detail page (`updateListingVariants` action; Etsy payload preview reflects the override). eslint/tsc/tests clean; CI green.
...
**Next action = the manual UI smoke only.** Phase 6's cloud integration smoke is DONE (2026-06-02 — see progress log): the real Design→variant-resolution→Listing→Printify path produced the correct 10-variant product, and the per-listing override narrowing was verified via the exact `getEtsyPayloadPreview` filter. No code changes were needed.
1. ~~**Cloud integration smoke**~~ — **DONE 2026-06-02.** 10/10 variant IDs resolved + Printify product created with them; override narrows 10→5 (white only); inherit/NULL path confirmed. Cloud restored to pre-smoke state.
2. **Manual UI smoke (still TODO — needs a human at the dashboard):** run the dashboard, exercise the VariantPicker (Design review card) and VariantOverrideEditor (listing detail) — confirm selection persists across a Regen, and the override saves/clears (NULL = inherit). This is the only remaining Phase 6 item; it's UI-interaction validation that can't be driven headlessly.

   **⚠️ If you re-run the agents for the UI smoke against the live shop:** the Listing agent drains the ENTIRE claimable queue. Confirm/park ALL pending + pending_publish listings AND approved designs without listings first — a status-count snapshot is not enough (it missed 2 claimable rows during the 2026-06-02 smoke, which created stray Printify products that then had to be cleaned up).

Everything before the manual UI smoke is shipped on `main` and validated. No code changes expected.

**⚙ Workflow notes captured (apply to any further variant work):** (a) verify steps editing Python MUST run `ruff check packages/` + `pyright packages/`, not just `pytest` (Phase 2's agent typed `db: object` + left a dead import; TS-only `npm run typecheck` missed it). (b) Rebuild `shared` dist before any TS typecheck (stale-`dist` footgun). (c) For the dashboard, the Opus review earns its cost on UI logic + a11y + the NULL-into-NOT-NULL class of bug (caught in Phase 4).
