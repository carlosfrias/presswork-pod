...
**Status:** Phases 0–5 COMPLETE ✅ — only Phase 6 (cloud integration smoke + manual UI smoke) remains.
**Date:** 2026-05-30 (Phases 1–5 shipped 2026-05-31)
...
**Next action = Phase 6 only** (the whole feature is built + CI-green; this is end-to-end validation against the cloud):
1. **Cloud integration smoke** (`ETSY_MOCK_MODE=true`, real cloud Supabase + real Printify): create/approve a brief with `shirt_colors=['White','Black']`, `shirt_sizes=['S','M','L','XL','2XL']` → run Design (`cd packages/design && python main.py`) → confirm `design_packages.printify_variant_ids` holds ~10 resolved IDs → run Listing → confirm the Printify product is created with those variants and `design_packages.printify_variants` has the rows → confirm the dashboard Etsy payload preview shows the Black/White × sizes grid. Then set a per-listing override (deselect Black) and confirm the preview narrows + Recreate re-creates the product with the subset.
2. **Manual UI smoke**: run the dashboard, exercise the VariantPicker (Design review) and VariantOverrideEditor (listing detail) — confirm selection persists across a Regen, and the override saves/clears (NULL = inherit).

Everything before Phase 6 is shipped on `main`. No code changes expected in Phase 6 unless the smoke surfaces a bug.
