...
**Status:** Phases 0–3 COMPLETE ✅ — Phases 4–6 (dashboard UI + integration smoke) pending.
**Date:** 2026-05-30 (Phases 1–3 shipped 2026-05-31)
...
**Next action = Phases 4 & 5** (dashboard UI — the only operator-facing piece left; backend resolve+override are live):
1. **Phase 4 — Dashboard Design surface.** New `VariantPicker` (colors multi-select + sizes multi-select), options from `printify_variant_catalog` (distinct colors/sizes for blueprint 145 / provider 39). Mount in `DesignReviewCard.tsx` after `BgRemovalPicker` (~`:416`), init from brief's `shirt_colors`/`shirt_sizes`. Serialize as hidden JSON inputs on the Regen + Approve forms (same pattern as `color_palette` ~`:477`). `regenerateDesign` (`design.ts:276`) + `approveDesign` (`:123`) parse + write to `trend_briefs`; expose on `DesignReviewItem` (`lib/queries/design.ts:18`).
2. **Phase 5 — Dashboard Listing surface (per-listing override).** New `VariantOverrideEditor` in `app/listings/[id]/page.tsx` Actions card (shown for `needs_review` | `pending_publish`). Render the inherited set (from `design_packages.printify_variant_ids` + catalog labels) with checkboxes; new server action `updateListingVariants` writes `listings.selected_variant_ids` (or NULL to reset). Pair with the existing "Recreate Printify product" button (re-queues to `pending` → Listing agent re-creates with the subset). Note: changing colors → mockups must regenerate.
Then **Phase 6** (tests + cloud integration smoke: brief with `shirt_colors=['White','Black']` → Design writes ~10 IDs → Listing in `ETSY_MOCK_MODE=true` creates Printify product → `printify_variants` has the rows → Etsy inventory preview shows the Black/White × sizes grid).

**⚙ Workflow note for Phases 4–6:** the verify step must run the **Python lint+type gates** (`ruff check packages/` + `pyright packages/`), not just `pytest` — Phase 2's build agent typed `db: object` and left an unused import; `npm run typecheck` is TS-only so it sailed through verify and was only caught by a manual `pyright` pass before commit. Also rebuild `shared` dist before any TS typecheck (stale-`dist` footgun).
